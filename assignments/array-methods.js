...
// Problem 1
// Create a new array with just the company name and the donation amount
let donorsWithAmounts = [];
runners.forEach(runner => {
  donorsWithAmounts.push(`${runner.company_name} - $${runner.donation}`);
});
console.log(donorsWithAmounts);

// Problem 2
// The biggest donors get special recognition in the program
// Use .filter() to get all donors giving 250 or more
let biggestDonors = runners.filter(runner => runner.donation >= 250);
console.log(biggestDonors);

// Problem 3
// A local company has agreed to match all donations >= 150
let donationMatch = runners.map(runner => {
  return {
    company_name: runner.company_name,
    donation: runner.donation,
    donation_match: runner.donation >= 150 ? runner.donation : 0,
  };
});
